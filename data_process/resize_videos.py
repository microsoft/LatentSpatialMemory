#!/usr/bin/env python3
"""
Resize training videos to target resolution (480x832).

This script fixes videos that were saved at MapAnything's processed resolution
instead of the target clip resolution.

Usage:
    python -m data_process.resize_videos
    python -m data_process.resize_videos --data-root data/Spatia/frame33_fps16_2000
"""
...
# Videos to resize (training outputs that may be at wrong resolution)
VIDEOS_TO_RESIZE = [
...
def get_video_info(video_path: Path) -> tuple[int, int, float] | None:
    """Get video resolution (width, height) and fps. Returns None if can't read."""
    try:
...
def resize_video(
    ...
    try:
        ...
        try:
            save_mp4(resized_frames, temp_path, fps=fps)
            # Verify the temp file is readable
            test_cap = cv2.VideoCapture(str(temp_path))
...
def main():
    parser = argparse.ArgumentParser(
        description="Resize training videos to target resolution"
    )
    parser.add_argument(
        "--data-root",
        type=str,
