...
from latent_mem.configs import DataConfig  # noqa: E402
from latent_mem.dataset import SpatiaLMDBDataset, spatia_collate_fn  # noqa: E402
from latent_mem.spatia.wan_video_new import (  # noqa: E402
    WanVideoPipeline,
    model_fn_wan_video,
)
...
CHECKPOINT_FORMAT = "latent_mem_finetune_v1"
LEGACY_CHECKPOINT_FORMAT = "spatia_original_finetune_v1"
RESUMABLE_CHECKPOINT_FORMATS = {CHECKPOINT_FORMAT, LEGACY_CHECKPOINT_FORMAT}
...
class LatentMemoryTrainingModel(nn.Module):
    def __init__(
        self,
        dit: nn.Module,
        vace: nn.Module,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional VACE-only initialization checkpoint, including official Spatia VACE weights.",
    )
    parser.add_argument(
        "--init-lora-checkpoint",
        type=Path,
        default=None,
        help="Optional LoRA-only initialization checkpoint, including official Spatia LoRA weights.",
    )
    parser.add_argument(
        "--stage",
        type=str,
...
def validate_checkpoint_args(args: argparse.Namespace) -> None:
    if args.resume_checkpoint is None:
        return
    if args.init_vace_checkpoint is not None or args.init_lora_checkpoint is not None:
        raise ValueError(
            "--resume-checkpoint restores model weights and optimizer state; "
            "do not combine it with --init-vace-checkpoint or --init-lora-checkpoint."
        )
...
def load_training_weights(
    model: nn.Module, path: Path, *, load_lora: bool
) -> dict[str, Any]:
    checkpoint = open_checkpoint(path)
    load_vace_weights(model, checkpoint, path)
    if load_lora:
        load_lora_weights(model, checkpoint, path)
    return checkpoint
...
def build_dataloader(args: argparse.Namespace) -> DataLoader:
    ...
    dataset = SpatiaLMDBDataset(data_config, model_version=args.model_version)
    return DataLoader(
        dataset,
...
def init_loss_history(
    *,
    ...
    if not isinstance(history, list):
        raise ValueError(f"Loss history must be a JSON array: {path}")

    filtered = []
    for item in history:
        if not isinstance(item, dict):
            continue
        if int(item.get("global_step", -1)) <= resume_global_step:
            filtered.append(item)
    if len(filtered) != len(history):
        save_loss_history(path, filtered)
    return filtered


def maybe_resume_optimizer(
    *,
    ...
    if not checkpoint or checkpoint.get("format") not in RESUMABLE_CHECKPOINT_FORMATS:
        return 0
    ...
    return int(checkpoint.get("stage_step", 0))
...
def train_stage(
    *,
    ...
    start_step = maybe_resume_optimizer(
...
def main() -> None:
    ...
    model = LatentMemoryTrainingModel(
        dit=dit,
        vace=pipe.vace,
    ...
    if args.resume_checkpoint is not None:
        resume_checkpoint = load_training_weights(
            model,
            args.resume_checkpoint,
            load_lora="lora" in stages,
        )
    if args.init_vace_checkpoint is not None:
        load_vace_weights(
            model,
            open_checkpoint(args.init_vace_checkpoint),
    ...
    if args.init_lora_checkpoint is not None:
        ...
        load_lora_weights(
            model,
            open_checkpoint(args.init_lora_checkpoint),
    ...
    if (
        resume_checkpoint
        and resume_checkpoint.get("format") in RESUMABLE_CHECKPOINT_FORMATS
    ):
