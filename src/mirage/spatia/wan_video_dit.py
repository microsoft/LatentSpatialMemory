...
logger.info(f"FLASH_ATTN_3_AVAILABLE: {FLASH_ATTN_3_AVAILABLE}")
logger.info(f"FLASH_ATTN_2_AVAILABLE: {FLASH_ATTN_2_AVAILABLE}")
logger.info(f"SAGE_ATTN_AVAILABLE: {SAGE_ATTN_AVAILABLE}")

if (
    not FLASH_ATTN_3_AVAILABLE
    and not FLASH_ATTN_2_AVAILABLE
    and not SAGE_ATTN_AVAILABLE
):
    logger.warning(
        "No flash attention library found, using scaled dot product attention"
    )
...
def sinusoidal_embedding_1d(dim, position, dtype=torch.float64):
...
def precompute_freqs_cis_3d(
    dim: int, start: int = 0, end: int = 1024, theta: float = 10000.0
):
    f_freqs_cis = precompute_freqs_cis(dim - 2 * (dim // 3), start, end, theta)
    h_freqs_cis = precompute_freqs_cis(dim // 3, start, end, theta)
    w_freqs_cis = precompute_freqs_cis(dim // 3, start, end, theta)
...
def precompute_freqs_cis(
    ...
    indices = torch.arange(start, end, device=freqs.device)
    if inverse:
...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        add_geo_feat_adapter: bool = False,
        in_dim_control_adapter: int = 24,
        in_dim_geo_feat_adapter: int = 1536,
        ...
        self.freqs = precompute_freqs_cis_3d(head_dim, start=0, end=1024)
        self.ref_freqs = precompute_freqs_cis_3d(head_dim, start=-1024, end=0)
        self.ref_freqs = (self.ref_freqs[0], self.freqs[1], self.freqs[2])
