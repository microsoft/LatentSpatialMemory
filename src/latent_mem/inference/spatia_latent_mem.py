...
from latent_mem.inference.utils import compute_iteration_plan
from latent_mem.latent_point_cloud import LatentPointCloud
from latent_mem.spatia.flow_match import FlowUniPCMultistepScheduler
from latent_mem.spatia.utils import load_state_dict
from latent_mem.spatia.wan_video_new import WanVideoPipeline, model_fn_wan_video
...
@dataclass
class SpatiaOriginalLatentMemConfig:
    num_frames: int = 33
    start_frame: int = 0
    infer_steps: int = 20
...
def validate_original_spatia_pipe(pipe: WanVideoPipeline) -> None:
    if pipe.dit is None:
    ...
    if not getattr(pipe.dit, "fuse_vae_embedding_in_latents", False):
        raise ValueError("Original Spatia inference requires fused-latent DiT.")
    if not getattr(pipe.dit, "seperated_timestep", False):
        raise ValueError("Original Spatia inference requires separated timesteps.")
    ...
    if int(patch_embedding.in_channels) != 96:
        raise ValueError(
            "Only the original Spatia 96-channel VACE path is supported, "
            f"got {patch_embedding.in_channels} channels."
        )


class SpatiaOriginalLatentMemPipeline:
    def __init__(
        self,
        pipe: WanVideoPipeline,
        config: SpatiaOriginalLatentMemConfig,
    ) -> None:
        self.pipe = pipe
        self.config = config
    ...
    def generate_single_iteration(
        ...
        *,
        ...
        for step_idx, timestep in enumerate(
            ...
            if step_idx == 0:
                print(
                    "Original Spatia latent inference shapes: "
...
def infer_mapanything_depths(
    *,
    ...
    model_id: str,
    device: torch.device,
    model_cache: SpatiaOriginalLatentMemPipeline,
