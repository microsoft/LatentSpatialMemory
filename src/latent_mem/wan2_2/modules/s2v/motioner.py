        ...
        for i in range(batch_size):
            ...
            if seq_len > 0:
                ...
                x_i = torch.view_as_complex(
                    x[i, seq_bucket[-1] : seq_bucket[-1] + seq_len]
                    .to(torch.float64)
...
class MotionerAttentionBlock(nn.Module):
    def __init__(
        ...
        if self_attn_block == "SelfAttention":
            ...
            self.self_attn = CasualSelfAttention(
                dim, num_heads, window_size, qk_norm, eps
            )
        ...
        if trainable_token_pos_emb:
            ...
            gride_sizes = [
                [
                    ...
                    torch.tensor([1, self.motion_side_len, self.motion_side_len])
                    .unsqueeze(0)
                    .repeat(1, 1),
                    torch.tensor([1, self.motion_side_len, self.motion_side_len])
                    .unsqueeze(0)
    ...
    def forward(
            ...
            for idx in sample_idx[0][::-1][:: self.sample_c]:
                tsm_frame_grid_sizes = [
                    [
                        ...
                        torch.tensor([idx + 1, seq_h, seq_w])
                        .unsqueeze(0)
                        .repeat(batch_size, 1),
                        torch.tensor([1, seq_h, seq_w])
                        .unsqueeze(0)
                        .repeat(batch_size, 1),
                    ]
                ]
        ...
        token_grid_sizes = [
            [
                ...
                .repeat(batch_size, 1),
                torch.tensor(
                    [1 if not self.trainable_token_pos_emb else -1, seq_h, seq_w]
                )
                .unsqueeze(0)
                .repeat(batch_size, 1),
        ...
        token_unpatch_grid_sizes = torch.stack(
            [torch.tensor([1, 32, 32], dtype=torch.long) for b in range(batch_size)]
        )
    ...
    def forward(self, motion_latents, add_last_motion=2):
        ...
        for m in motion_latents:
            ...
            padd_lat = torch.zeros(
                16, self.zip_frame_buckets.sum(), lat_height, lat_width
            ...
            grid_sizes = (
                ...
                if add_last_motion < 2 and self.drop_mode == "drop"
                    ...
                    [
                        ...
                        .repeat(1, 1),
                        torch.tensor(
                            [self.zip_frame_buckets[0], lat_height // 2, lat_width // 2]
                        )
                        .unsqueeze(0)
                        .repeat(1, 1),
                    ]
            ...
            grid_sizes_2x = (
                ...
                if add_last_motion < 1 and self.drop_mode == "drop"
                    ...
                    [
                        ...
                        .repeat(1, 1),
                        torch.tensor(
                            [self.zip_frame_buckets[1], lat_height // 2, lat_width // 2]
                        )
                        .unsqueeze(0)
                        .repeat(1, 1),
                    ]
