...
def _parse_args():
    ...
    parser.add_argument(
        "--k", type=int, default=7, help="Number of kernel size for mask dilation."
    )
    parser.add_argument(
        "--w_len",
        type=int,
...
if __name__ == "__main__":
    ...
    assert len(args.resolution_area) == 2, (
        "resolution_area should be a list of two integers [width, height]"
    )
    assert not args.use_flux or args.retarget_flag, (
        "Image editing with FLUX can only be used when pose retargeting is enabled."
    )
    ...
    sam2_checkpoint_path = (
        os.path.join(args.ckpt_path, "sam2/sam2_hiera_large.pt")
        if args.replace_flag
        else None
    )
    flux_kontext_path = (
        os.path.join(args.ckpt_path, "FLUX.1-Kontext-dev") if args.use_flux else None
    )
