...
def recover_pinhole_intrinsics_from_ray_directions(
    ...
    if is_high_res or use_geometric_calculation:
        ...
        # Solve the system for each batch (bf16/fp16 not supported by linalg.solve)
        solve_dtype = (
            torch.float32
            if ATA_x.dtype in (torch.float16, torch.bfloat16)
            else ATA_x.dtype
        )
        ATA_x_solve = ATA_x.to(solve_dtype)
        ATb_x_solve = ATb_x.to(solve_dtype)
        device_type = "cuda" if ATA_x_solve.is_cuda else "cpu"
        with torch.autocast(device_type=device_type, enabled=False):
            solution_x = torch.linalg.solve(ATA_x_solve, ATb_x_solve).squeeze(
                2
            )  # [B, 2]
        ...
        ATA_y_solve = ATA_y.to(solve_dtype)
        ATb_y_solve = ATb_y.to(solve_dtype)
        with torch.autocast(device_type=device_type, enabled=False):
            solution_y = torch.linalg.solve(ATA_y_solve, ATb_y_solve).squeeze(
                2
            )  # [B, 2]
