...
from util.finetune_utils import finetune_decoder, finetune_depth_model
...
def run(
    config, start_keyframe, inpainting_prompt_list, cameras, cameras_interp, helper
):
    ###### ------------------ Load modules ------------------ ######

    if config["skip_gen"]:
        kfgen_save_folder = (
            Path(config["runs_dir"]) / f"{config['kfgen_load_dt_string']}_kfgen"
        )
    else:
    ...
    all_renderedkeyframes = [start_keyframe]
    all_interpframes = [start_keyframe]
    gt_frames = []

    pt_gen = TextpromptGen(config["runs_dir"], False)
    ...
    inpainter_pipeline.scheduler = DPMSolverMultistepScheduler.from_config(
    ...
    for i in range(helper.num_scenes):
        # if config['use_gpt']:
        ...
        for j in range(1):
            ...
            if config["skip_gen"]:
                kf_gen_dict = torch.load(
                    kfgen_save_folder / f"s{i:02d}_k{j:01d}_gen_dict.pt"
                )
                kf1_depth, kf2_depth = (
                    kf_gen_dict["kf1_depth"],
                    kf_gen_dict["kf2_depth"],
                )
                kf1_image, kf2_image = (
                    kf_gen_dict["kf1_image"],
                    kf_gen_dict["kf2_image"],
                )
                kf1_camera, kf2_camera = (
                    kf_gen_dict["kf1_camera"],
                    kf_gen_dict["kf2_camera"],
                )
                kf2_mask = kf_gen_dict["kf2_mask"]
                inpainting_prompt, adaptive_negative_prompt = (
                    kf_gen_dict["inpainting_prompt"],
                    kf_gen_dict["adaptive_negative_prompt"],
                )
                rotation = kf_gen_dict["rotation"]
            else:
                rotation = rotation_path[i * config["num_keyframes"] + j]
                kf1_kf2_cameras = cameras[
                    i * config["num_keyframes"] + j : i * config["num_keyframes"]
                    + j
                    + 2
                ]
                ...
                for regen_id in range(config["regenerate_times"] + 1):
                    ...
                    # depth_model, _, _, _ = load_model(torch.device("cuda"), 'dpt_beit_large_384.pt', 'dpt_beit_large_384', optimize=False)
                    ...
                    kf_gen = KeyframeGen(
                        ...
                        start_keyframe,
                        inpainting_prompt,
                        regen_negative_prompt + adaptive_negative_prompt,
                        segment_model=segment_model,
                        segment_processor=segment_processor,
                        kf1_kf2_cameras=kf1_kf2_cameras,
                    ).to(config["device"])
                    save_root = Path(kf_gen.run_dir) / "images"
                    kf_idx = 0

                    save_depth_map(
                        kf_gen.depths[kf_idx].detach().cpu().numpy(),
                        save_root / "kf1_original",
                        vmin=0,
                        vmax=vmax,
                    )
                    kf_gen.refine_disp_with_segments(
                        kf_idx, background_depth_cutoff=cutoff_depth
                    )
                    save_depth_map(
                        kf_gen.depths[kf_idx].detach().cpu().numpy(),
                        save_root / "kf1_processed",
                        vmin=0,
                        vmax=vmax,
                    )
                    ...
                    if (
                        config["enable_regenerate"]
                        and regen_id <= config["regenerate_times"] - 1
                    ):
                        gpt_border, gpt_blur = pt_gen.evaluate_image(
                    ...
                    if regen_id == config["regenerate_times"] - 1:
                        print(
                            "Regenerating faild after {} times".format(
                                config["regenerate_times"]
                            )
                        )
                        if gpt_border:
                ...
                if config["finetune_decoder_gen"]:
                    ToPILImage()(inpaint_output["inpainted_image"].detach()[0]).save(
                        save_root / "kf2_before_ft.png"
                    )
                    finetune_decoder(
                        config,
                        kf_gen,
                        render_output,
                        inpaint_output,
                        config["num_finetune_decoder_steps"],
                    )

                kf_gen.update_images_and_masks(
                    inpaint_output["latent"], render_output["inpaint_mask"]
                )

                kf2_depth_should_be = render_output["rendered_depth"]
                mask_to_align_depth = ~(render_output["inpaint_mask_512"] > 0) & (
                    kf2_depth_should_be < cutoff_depth + kf_gen.kf_delta_t
                )
                ...
                if config["finetune_depth_model"]:
                    finetune_depth_model(
                        ...
                        cutoff_depth=cutoff_depth + kf_gen.kf_delta_t,
                    )
                with torch.no_grad():
                    kf2_ft_depth_original, kf2_ft_disp_original = kf_gen.get_depth(
                        kf_gen.images[kf_idx]
                    )
                    (
                ...
                kf_gen.refine_disp_with_segments(
                    kf_idx, background_depth_cutoff=cutoff_depth + kf_gen.kf_delta_t
                )
                save_depth_map(
                    kf_gen.depths[-1].cpu().numpy(),
                    save_root / "kf2_ft_depth_processed",
                    vmin=0,
                    vmax=vmax,
                )
                ...
                # if i * config['num_keyframes'] + j == 0:
                #     render_output_tmp = kf_gen.render(kf_idx, kf1_camera, save=False)
                #     all_renderedkeyframes.append(ToPILImage()(render_output_tmp["rendered_image"][0]))
                #     all_interpframes.append(ToPILImage()(render_output_tmp["rendered_image"][0]))

                # render_output_tmp = kf_gen.render(kf_idx, kf2_camera, save=False)
                # all_renderedkeyframes.append(ToPILImage()(render_output_tmp["rendered_image"][0]))
                ...
                kf_gen_dict = {
                    ...
                    "kf1_image": kf1_image,
                    "kf2_image": kf2_image,
                    "kf1_camera": kf1_camera,
                    "kf2_camera": kf2_camera,
                    "kf2_mask": kf2_mask,
                ...
                torch.save(
                    kf_gen_dict, kfgen_save_folder / f"s{i:02d}_k{j:01d}_gen_dict.pt"
                )
            ...
            try:
                is_next_rotation = (
                    rotation_path[i * config["num_keyframes"] + j + 1] != 0
                )
            except IndexError:
                is_next_rotation = False
            try:
                is_previous_rotation = (
                    rotation_path[i * config["num_keyframes"] + j - 1] != 0
                )
            except IndexError:
                is_previous_rotation = False
            is_beginning = i == 0 and j == 0
            speed_up = (rotation == 0) and (
                (is_last_scene and is_last_keyframe) or is_next_rotation
            )
            speed_down = (rotation == 0) and (is_beginning or is_previous_rotation)
            total_frames = helper.frames
            ...
            (
                ...
                kf2_mask_upsample,
                kf2_image_upsample,
            ) = kf_interp.upsample_kf2()

            kf_interp.update_additional_point_cloud(
                kf2_depth_upsample,
                kf2_image_upsample,
                valid_mask=kf2_mask_upsample,
                camera=kf2_camera_upsample,
                points_2d=kf_interp.points_kf2,
            )
            inconsistent_additional_point_index = kf_interp.visibility_check()
            kf2_depth_updated = kf_interp.update_additional_point_depth(
                inconsistent_additional_point_index,
                depth=kf2_depth_upsample,
            ...
            kf_interp.depths[0] = F.interpolate(
                kf2_depth_updated, size=(512, 512), mode="nearest"
            )
            ...
            for epoch in tqdm(range(1, total_frames)):
                start = (i * 1 + j) * (total_frames - 1)
                camera = cameras_interp[start + epoch]
                render_output_kf1 = kf_interp.render_kf1(epoch, camera)

                inpaint_output = kf_interp.inpaint(
                    render_output_kf1["rendered_image"],
                    render_output_kf1["inpaint_mask"],
                )
                ...
                kf_interp.update_images_and_masks(
                    inpaint_output["latent"], render_output_kf1["inpaint_mask"]
                )

                kf_interp.update_additional_point_cloud(
                    render_output_kf1["rendered_depth"],
                    kf_interp.images[-1],
                    append_depth=True,
                )
            ...
            all_interpframes.extend(kf_interp.output_images)
            print("interp images: ", len(kf_interp.output_images))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--example_config", default="./config/example.yaml")
    # specify the domain
    parser.add_argument(
    ...
    for data in dataloader:
        start_keyframe, inpainting_prompt_list, cameras, cameras_interp = helper.adapt(
            data
        )
